...
addListBtn.addEventListener('click', ()=>{
    // Список добавляем новую карточку с инпутами
    let list = Date.now()
    notesList.appendChild(getCardTemplateList(list, "", "", true))

    getCardBody(list).setAttribute('data-created', 'false')
...
notesList.addEventListener('click', function(e) {
    ...
    let idList = e.target.dataset.list
    ...
    if(e.target.classList.contains('btn-danger')) {
        ...
        console.log('save')
        if(getCardBody(idList).dataset.edit){
            editNoteList(idList)
        } else {
            createNoteList(idList)
        }
        ...
        console.log('edit')
        let currentCol = getCol(idList)
        let newCol = getCardTemplateList(idList, getTitleVal(idList, false), getTextValList(idList, false), true)
        currentCol.innerHTML = newCol.innerHTML
        getCardBody(idList).setAttribute("data-edit", "true");
...
<div class="form-inline">
            ...
            <div class="custom-control custom-checkbox my-1 mr-sm-2">
                ...
                <label class="custom-control-label" for="id">
                    <input class="form-control" id="note-text" data-set="set0" value="${text}">
    ...
    if(editStatus){
        submitBtn = `<button class="btn btn-primary save-btn" data-list="${id}">Save</button>`
        neededContentElems = inputElems
    } else{
        submitBtn = `<button class="btn btn-success edit-btn" data-id="${id}">Edit</button>`
        neededContentElems = textElems
    ...
    const cardContainer = `
            <div class="card">
                <div class="card-body bg-warning" data-list="${id}">
                    <div class="text-right">
                        <button type="button" data-list="${id}" class="btn btn-danger">-</button>
                    </div>
                    ${neededContentElems}
                    ${submitBtn}          
                </div>
